...
Real slicing is performed in the browser by the open-source Kiri:Moto engine. Model data stays in the browser and is not uploaded to a slicing service. The engine is loaded as web code from Grid.Space when it is first needed.
...
The deployed site must be allowed to load `https://grid.space/code/engine.js`. For a fully offline deployment, self-host a matching Kiri:Moto engine build and change `KIRI_ENGINE_URL` in `src/slicing/kiriEngine.ts`.
...
Kiri:Moto is an open-source Grid.Space project distributed under the MIT license. See the [Kiri:Moto documentation](https://docs.grid.space/kiri-moto/) and [GridSpace/grid-apps repository](https://github.com/GridSpace/grid-apps).
